...
class HearTrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_indx):
        ...
        loss_strong = self.supervised_loss(strong_preds[strong_mask], labels_strong_SGP)
    ...
    def validation_step(self, batch, batch_indx):
        ...
        if torch.any(mask_SONYC):
            ...
            loss_weak = self.supervised_loss(weak_preds[mask_SONYC], labels_weak_SONYC)
        ...
        if torch.any(mask_SGP):
            ...
            # supervised loss on strongly labelled turned into weakly
            loss_strong = self.supervised_loss(strong_preds[mask_SGP], labels[mask_SGP])
        ...
        self.update_metrics(
            "Val", strong_preds, weak_preds, labels, filenames, mask_SGP, mask_SONYC
        )
    ...
    def test_step(self, batch, batch_indx):
        ...
        if torch.any(mask_SGP):
            ...
            loss_strong = self.supervised_loss(
                strong_preds[mask_SGP], labels_all[mask_SGP]
            )
        ...
        if torch.any(mask_near_SONYC):
            self.weak_metrics["Test"]["SONYC_near"].update(
                weak_preds_near[mask_near_SONYC],
        ...
        if torch.any(mask_far_SONYC):
            self.weak_metrics["Test"]["SONYC_far"].update(
                weak_preds_far[mask_far_SONYC], labels_weak_far[mask_far_SONYC].int()
            )
            self.weak_metrics_by_class["SONYC_far"].update(
                weak_preds_far[mask_far_SONYC], labels_weak_far[mask_far_SONYC].int()
            )
            self.update_df_metrics(
                weak_preds_far[mask_far_SONYC],
        ...
        if torch.any(mask_near_SGP):
            self.weak_metrics["Test"]["SGP_near"].update(
                weak_preds_near[mask_near_SGP], labels_weak_near[mask_near_SGP].int()
            )
            self.weak_metrics_by_class["SGP_near"].update(
                weak_preds_near[mask_near_SGP], labels_weak_near[mask_near_SGP].int()
            )
            self.update_df_metrics(
                weak_preds_near[mask_near_SGP],
        ...
        if torch.any(mask_far_SGP):
            self.weak_metrics["Test"]["SGP_far"].update(
                weak_preds_far[mask_far_SGP], labels_weak_far[mask_far_SGP].int()
            )
            self.weak_metrics_by_class["SGP_far"].update(
                weak_preds_far[mask_far_SGP], labels_weak_far[mask_far_SGP].int()
            )
            self.update_df_metrics(
                weak_preds_far[mask_far_SGP],

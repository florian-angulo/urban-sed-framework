...
class TorchScaler(torch.nn.Module):
    def __init__(
        self, batch_sizes, statistic="dataset", normtype="standard", dims=(2,), eps=1e-8
    ):
        ...
        for batch in tqdm.tqdm(dataloader):
            ...
            if idx == 0:
                ...
                if torch.any(mask_strong):
                    mean_strong = (
                        torch.mean(features[mask_strong], self.dims, keepdim=True)
                        .mean(0)
                        .unsqueeze(0)
                    )
                    mean_squared_strong = (
                        torch.mean(features[mask_strong] ** 2, self.dims, keepdim=True)
                        .mean(0)
                        .unsqueeze(0)
                    )
                if torch.any(mask_weak):
                    mean_weak = (
                        torch.mean(features[mask_weak], self.dims, keepdim=True)
                        .mean(0)
                        .unsqueeze(0)
                    )
                    mean_squared_weak = (
                        torch.mean(features[mask_weak] ** 2, self.dims, keepdim=True)
                        .mean(0)
                        .unsqueeze(0)
                    )
                ...
                if torch.any(mask_unlab):
                    mean_unlab = (
                        torch.mean(features[mask_unlab], self.dims, keepdim=True)
                        .mean(0)
                        .unsqueeze(0)
                    )
                    mean_squared_unlab = (
                        torch.mean(features[mask_unlab] ** 2, self.dims, keepdim=True)
                        .mean(0)
                        .unsqueeze(0)
                    )
            else:
    ...
    def normalize(self, tensor, type_mask):
        if self.statistic == "dataset":
            ...
            tensor_without_outliers = torch.where(
                torch.abs(tensor - mean) <= std, tensor, torch.nan
            )

            nanmean = torch.nanmean(tensor_without_outliers, self.dims, keepdim=True)
            nanmean_squared = torch.nanmean(
                torch.square(tensor_without_outliers), self.dims, keepdim=True
            )

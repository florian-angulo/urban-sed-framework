...
class ConcatDatasetUrban(torch.utils.data.ConcatDataset):
    ...
    def __init__(
        ...
        if hear_encoder == "open_l3_6144":
            ...
            raise NotImplementedError(
                f"{hear_encoder} not currently preprocessed. Check the encoder name or preprocess the dataset."
            )
    ...
    def collate_fn(self, batch):
        ...
        if len(self.batch_sizes) > 1:
            ...
            if indx_strong > 0:
                ...
                embeddings[:indx_strong] = self.hdf5["SINGA-PURA"][self.hear_encoder][
            ...
            if indx_weak > 0:
                ...
                embeddings[indx_strong : indx_strong + indx_weak] = self.hdf5["SONYC"][
                    self.hear_encoder
            ...
            embeddings = np.zeros(
                (n_SONYC + n_SINGAPURA, self.n_frames, self.embed_size)
            )
            labels = np.zeros((n_SONYC + n_SINGAPURA, self.n_classes, self.n_frames_gt))
            mem_filenames = np.copy(filenames)

            if n_SINGAPURA > 0:
                embeddings[:n_SINGAPURA] = self.hdf5["SINGA-PURA"][self.hear_encoder][
            ...
            if n_SONYC > 0:
                embeddings[n_SINGAPURA:] = self.hdf5["SONYC"][self.hear_encoder][
    ...
    def get_rescale_matrix(self, n_frames_target, n_frames_hdf5=156):
        ...
        for i in range(r_matrix.shape[1]):
            lookup = min(
                n_frames_hdf5 - 1, int(i * n_frames_hdf5 / (n_frames_target * decim))
            )
            r_matrix[lookup, i] = 1

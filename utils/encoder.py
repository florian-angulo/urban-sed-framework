...
class ManyHotEncoder:
    ...
    def fine_to_coarse(self, prob):
        ...
        if prob.dim() == 3:
            return torch.max(prob.transpose(1,2)[:,:,:,None]*self.ftc_matrix,axis=-2,keepdims=True).values.squeeze(-2).transpose(1,2)
        elif prob.dim() == 2:
            return torch.max(prob[:,:,None]*self.ftc_matrix,axis=-2,keepdims=True).values.squeeze(-2)
    ...
    def decode_strong(self, labels, taxo_level="coarse"):

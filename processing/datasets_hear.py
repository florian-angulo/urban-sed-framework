...
class ConcatDatasetUrban(torch.utils.data.ConcatDataset):
    ...
    def collate_fn(self, batch):
        ...
        # rescale groundtruth if not open_l3
    ...
    def get_rescale_matrix(self, n_frames_target, n_frames_hdf5=156):
        ...
        elif n_frames_target < n_frames_hdf5:
            small_n_frames = n_frames_target
            large_n_frames = n_frames_hdf5
            transpose = True
        else:
            small_n_frames = n_frames_hdf5
            large_n_frames = n_frames_target
            transpose = False

        r_matrix = np.zeros((small_n_frames, large_n_frames))
        for i in range(r_matrix.shape[1]):
            lookup = min(small_n_frames - 1,int(i*small_n_frames/large_n_frames))
            r_matrix[lookup, i] = 1
